...
describe('ガードとローテーションの仕様（2026-07-23 社長指摘）', () => {
  it('ガードの軽減はガード使用者本人のみ（全体攻撃の他対象はフルダメージ）', () => {
    ...
    expect(state.players[1].characters[0].damage).toBe(Math.max(0, atk.baseValue - guard.baseValue)); // 本人は軽減
    expect(state.players[1].characters[1].damage).toBe(atk.baseValue); // 控えはフル
  });
...
describe('ヤクビの発動条件（2026-07-23 社長決定）', () => {
  it('控えがいない（生存が自分だけ）なら能力は発動しない', () => {
    const state = battleWith(['1-A009-SR', ORUS], VANILLA_ATK, [DADA, OWU], VANILLA_ATK);
    // 控えのオルスを戦闘不能にしておく
    const bench = state.players[0].characters[1];
    bench.damage = bench.maxHp;

    const seqBefore = state.eventSeq;
    applyAction(state, { type: 'endPlay' });
    endTurnCharging(state);
    expect(state.players[1].characters[0].damage).toBe(0); // 発動しない
    expect(state.players[0].actorIndex).toBe(0); // 交代もしない
    // 何も起きなかったパッシブは「発動」演出イベント自体が出ない（毎回光る問題の防止）
    const newEvents = state.events.slice(-(state.eventSeq - seqBefore));
    expect(newEvents.filter((e) => e.t === 'abilityTriggered')).toHaveLength(0);
  });
});
...
describe('型付きイベントストリーム（UI演出の唯一の情報源）', () => {
  it('ガード・ダメージ・交代イベントが正しい位置情報つきで発行される', () => {
    ...
    expect(guard).toMatchObject({
      ...
      before: atk.baseValue, after: Math.max(0, atk.baseValue - guardCard.baseValue),
    });
    // ダメージイベントは控え（1番）にだけ、フルの値で出る
    const damages = events.filter((e) => e.t === 'damage');
    expect(damages).toMatchObject([{ t: 'damage', player: 1, charIndex: 1, n: atk.baseValue }]);
    // 使用イベントは使用キャラの位置を持つ
...
describe('ロックと二重スイッチの修正（2026-07-23 社長指摘）', () => {
  it('ロック中は飛び出る（控えから使える）を控えから使えない', () => {
    ...
    expect(opts).toHaveLength(0); // 控えからの使用は選べない
  });
  ...
  it('反射で攻撃側アクターが強制交代したら、追加の自動ローテーションはしない', () => {
    ...
    applyAction(state, { type: 'playSkill', handIndex: 0 });
    // 強制交代で2番手(ストミー=1)へ。自動ローテーションが重なって2(ダダ)まで
    // 進んでしまわないこと
    expect(state.players[0].actorIndex).toBe(1);
  });
});
...
describe('控えだけを狙う攻撃とガード（2026-07-23 社長決定）', () => {
  it('コメットスナイプで控えを指定した時、相手はガードで割り込めない', () => {
    ...
    applyAction(state, { type: 'playSkill', handIndex: 0, targetIndex: 1 }); // 控え(1)を指定
    expect(state.phase).toBe('play'); // guardフェーズにならず即解決
    expect(state.players[1].characters[1].damage).toBe(skill.baseValue);
  });
});
...
describe('強制チャージルール（2026-07-23 社長決定）', () => {
  it('手札5枚以上のままターンは終えられない。1枚チャージすれば終えられる', () => {
    const state = battleWith([ORUS], VANILLA_ATK, [DADA, OWU], VANILLA_ATK);
    applyAction(state, { type: 'endPlay' });
    expect(state.players[0].hand).toHaveLength(5);

    // endTurn が合法手に無い & 実行しても弾かれる
    expect(legalActions(state).some((a) => a.type === 'endTurn')).toBe(false);
    expect(() => applyAction(state, { type: 'endTurn' })).toThrow();
  ...
  it('手札4枚以下ならチャージなしでターンを終えられる', () => {
    ...
    if (state.phase === 'guard') applyAction(state, { type: 'pass' });
    applyAction(state, { type: 'endPlay' });
    expect(state.players[0].hand.length).toBeLessThan(5);
    applyAction(state, { type: 'endTurn' });
    expect(state.active).toBe(1);
  });
});
