 ...
 *
 * 実装できていないもの（理由つき）:
 * - 1-A006 アニマ「アクターにできる」: プレイヤーの任意選択が必要（選択UI/AI対応後に実装）
 * - 1-A024 ロッソ / 1-A073 ポイントブレイクの装備部分: 装備システムが未実装（ルール未決定）
 * - 1-A054 炎霊召喚 / 1-A099 風を集める: 「デッキから使用」は簡易版（該当する攻撃スキルの
 *   基本値ぶんのダメージを直接与える。ガード割り込みなし）
 */
...
export const VOL1_EFFECTS: Record<string, CardEffect> = {
  ...
  '1-A067-R': {
    ...
    targeting: 'all',
    onAttackResolved: (api, dealt) => {
      if (dealt > 0) api.discardEnemyAp(Math.min(3, dealt)); // ダメージを与えたキャラ数ぶん（近似）
    },
  },
  '1-A069-R': {
    kind: 'skill',
