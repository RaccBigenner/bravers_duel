...
- 最後の有効commandから**30分**操作が無いと、サーバーは試合を`suspended`にしてsnapshotを永続化し、
  Durable Objectを解放する
- `suspended`はプレイヤーから見た敗北・ペナルティを一切伴わない。再開すると同じ盤面から続けられる
...
再接続猶予（90秒）とチェスクロックは**別軸で同時に働く**。どちらが先に切れても、
先に切れた方の処理が優先される。

- 切断した瞬間がそのプレイヤーの意思決定中なら、チェスクロックは切断中も減り続ける。
  90秒が残っていてもチェスクロックが0になれば、その時点で即座に`TIMEOUT`敗北が確定する
  （サーバーは再接続を待たない）
- 切断した瞬間が相手の意思決定中（自分の番ではない）なら、チェスクロックは止まったまま。
  ただし**90秒の再接続猶予を過ぎたら**、それ以降にそのプレイヤーへ意思決定が回ってきた回は、
  通常の60秒/30秒を待たずに**即座に**自動処理（pass/skip/endPlay）を適用する。
  猶予切れ後は「待っても来ない」と確定しているため、対戦相手を待たせ続けない
- 上記いずれの経路でも、連続2回/合計3回の大きな時間切れで敗北する既存ルールへ合流する
...
実害を「最大8分（標準プリセット時）」に頭打ちする設計になっている。追加の検知は
