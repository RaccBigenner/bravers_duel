...
公開前チェック（画像・効果の実装・基本値・ID重複）が **全部緑になるまで押せない**。
押すと、制作中のカードと画像が公開リポジトリへ移り、数分でゲームに出る。
...
`/api/master` は **画像の一覧（ファイル名と版番号）もまとめて返す**。管理画面はこれで
